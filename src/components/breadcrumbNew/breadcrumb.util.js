export function getBread(pageName, query, data, groupingTitle) {
  switch (pageName) {
    case 'song':
    ...
    case 'albums':
      return getGroupingParts(groupingTitle, pageName);
    case 'collaborators':
      return getGroupingParts(groupingTitle, pageName);
    case 'credits':
      return getGroupingParts(groupingTitle, pageName);
    case 'activity':
      return getActivityParts(data);
    default:
...
function getActivityParts(data) {
  return [{ label: 'Catalog'}, { label: data?.trackTitle}]
}


function getGroupingParts(groupingTitle, pageName) {
  return [{ label: 'Catalog'}, { label: pageName}, { label: groupingTitle}]
}