...
export default {
  ...
  watch: {
    ...
    groupBy() {
      if (this.playlistTrackId.length) {
        this.fetchActivity()
      }
    },
    timePeriod() {
  ...
  computed: {
    activity() {
      return this.$store.getters['analytics/activityOverview/playlistData']
    },
    playlistTrackId() {
  ...
  methods: {
    ...
    getPlaylistsActivity() {
      return this.$store.dispatch('analytics/activityOverview/getPlaylistActivity', {
        id: this.profileId,
        trackId: this.playlistTrackId,
