<template>
  <master-table>
    ...
    />
    <div v-else-if="!pagination.loading" class="zero-static zero-static--light zero-state__table">
      <base-icon name="list"/>
...
import { commonColumns } from 'components/analytics/catalog-table.vue';
import SortButton from 'components/ui/sort-button.vue';

export default {
  name: 'catalog-master-table',
  components: {
    MasterTable,
    MasterHeader,
    MasterSearch,
  ...
  methods: {
    ...
    setGroup(value) {
      ...
      this.$store.commit('analytics/grouping/setGroup', value)
    },
